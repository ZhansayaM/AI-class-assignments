...
def UCSearch(start, goal, barrier):
    ...
    while len(fringe) > 0:
        ...
        ############################################################################################
        if curNode != goal:
            if curNode not in expandedNodes:                                                                # if the current node was not expanded yet, we can expand it
                expandedNodes.add(curNode)                                                                  # and add the current node to the list of expanded nodes
                for neighbor in get_neighbors(curNode):                                                     # iterate through the neighbors of the current node
                    if neighbor not in expandedNodes:                                                       # and other actions are held only if the neighbor is not in the expanded nodes list. For example, it may be current node's parrent, which is already expanded
                        if neighbor not in fringe:                                                          # if the neighbor node is not in the fringe, we need to add it there
                            fringe.add(neighbor)                                                            # add the neighbor to the fringe list
                            G[neighbor] = G[curNode] + move_cost(neighbor, barrier)                         # calculate and store the value of the path cost and add the previous node's cost to the current
                            parent[neighbor] = curNode                                                      # assign the parent of the neighbor as the current node
                        elif neighbor in fringe and move_cost(neighbor,barrier) + curGscore < G[neighbor]:  #if the node is already in fringe, we need to update its value, if the current value is less than its prev value
                            G[neighbor] = G[curNode] + move_cost(neighbor,barrier)                          # calculate and store the value of the path cost and add the previous node's cost to the current
                            parent[neighbor] = curNode                                                      # assign the parent of the neighbor as the current node
        ###########################################################################################
        ##                                   END OF YOUR CODE                                    ##
                ...
                for neighbor in get_neighbors(curNode):                                                     # iterate through the neighbors of the current node
                    if neighbor not in expandedNodes:                                                       # and other actions are held only if the neighbor is not in the expanded nodes list. For example, it may be current node's parrent, which is already expanded
                        if neighbor not in fringe:                                                          # if the neighbor node is not in the fringe, we need to add it there
                            fringe.add(neighbor)                                                            # add the neighbor to the fringe list
                            G[neighbor] = G[curNode] + move_cost(neighbor,barrier)                          # calculate and store the value of the path cost and add the previous node's cost to the current
                            F[neighbor] = G[neighbor] + heuristic(neighbor,goal)                            # calculate and store the A* value by the cost of the path and the heuristic value
                            parent[neighbor] = curNode  # assign the parent of the neighbor as the current node
                        elif neighbor in fringe and move_cost(neighbor, barrier) + curFscore < G[neighbor]: #if the node is already in fringe, we need to update its value, if the current value is less than its prev value
                            G[neighbor] = G[curNode] + move_cost(neighbor, barrier)                         # calculate and store the value of the path cost and add the previous node's cost to the current
                            F[neighbor] = G[neighbor] + heuristic(neighbor,goal)                            # calculate and store the A* value by the cost of the path and the heuristic value
                            parent[neighbor] = curNode                                                      # assign the parent of the neighbor as the current node
        ###########################################################################################
